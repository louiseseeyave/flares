...
#SBATCH -A dp004
#SBATCH -p cosma7
#SBATCH --job-name=get_REF
#SBATCH --array=0
#SBATCH -t 0-02:00
#SBATCH --ntasks-per-node=8
#SBATCH -o logs/init_std_output.%J
#SBATCH -e logs/init_std_error.%J


module purge
# module load gnu_comp/7.3.0 openmpi/3.0.1 hdf5/1.10.3 python/3.6.5
module load gnu_comp/7.3.0 openmpi/3.0.1 hdf5/1.10.3 pythonconda3/2020-02
...
# for ii in ${array[@]}
#   do
#     mpiexec -n $SLURM_NTASKS python3 -m mpi4py download_methods.py $SLURM_ARRAY_TASK_ID $ii FLARES $output_folder
# done
...
for ii in ${array[@]}
  do
    mpiexec -n $SLURM_NTASKS python3 -m mpi4py download_methods.py 0 $ii REF $output_folder
done
