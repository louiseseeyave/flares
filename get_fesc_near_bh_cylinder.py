...
def save_data(newfile, rg, snap, nstellar_5pkpc, nsf_bhids, nsf_bhmass,
    ...
    with h5py.File(newfile, 'a') as fl:
        ...
        if f'{rg}/{snap}/Particle/BH_nstellar_{radius}pkpc_cylinder' in fl:
            print(f'deleting {rg}/{snap}/Particle/BH_nstellar_{radius}pkpc_cylinder')
            del fl[f'{rg}/{snap}/Particle/BH_nstellar_{radius}pkpc_cylinder']

        if f'{rg}/{snap}/Particle/BH_mstellar_{radius}pkpc_cylinder' in fl:
            print(f'deleting {rg}/{snap}/Particle/BH_mstellar_{radius}pkpc_cylinder')
            del fl[f'{rg}/{snap}/Particle/BH_mstellar_{radius}pkpc_cylinder']

        if f'{rg}/{snap}/Particle/BH_ngas_{radius}pkpc_cylinder' in fl:
            print(f'deleting {rg}/{snap}/Particle/BH_ngas_{radius}pkpc_cylinder')
            del fl[f'{rg}/{snap}/Particle/BH_ngas_{radius}pkpc_cylinder']

        if f'{rg}/{snap}/Particle/BH_mgas_{radius}pkpc_cylinder' in fl:
            print(f'deleting {rg}/{snap}/Particle/BH_mgas_{radius}pkpc_cylinder')
            del fl[f'{rg}/{snap}/Particle/BH_mgas_{radius}pkpc_cylinder']

        if f'{rg}/{snap}/Particle/BH_meanfesc_{radius}pkpc_cylinder' in fl:
            print(f'deleting {rg}/{snap}/Particle/BH_meanfesc_{radius}pkpc_cylinder')
            del fl[f'{rg}/{snap}/Particle/BH_meanfesc_{radius}pkpc_cylinder']

        # create datasets --------------------------------------------
        
        fl.create_dataset(f'{rg}/{snap}/Particle/BH_Mass_notsubfind', data = nsf_bhmass, dtype = np.float64)
        ...
        fl.create_dataset(f'{rg}/{snap}/Particle/BH_ID_notsubfind', data = nsf_bhids, dtype = np.int64)
        ...
        fl.create_dataset(f'{rg}/{snap}/Particle/BH_nstellar_{radius}pkpc_cylinder', data = nstellar_5pkpc, dtype = np.float64)
        fl[f'{rg}/{snap}/Particle/BH_nstellar_{radius}pkpc_cylinder'].attrs['Description'] = 'No. of stellar particles within 5pkpc of BH - NOT SUBFIND!'
        fl[f'{rg}/{snap}/Particle/BH_nstellar_{radius}pkpc_cylinder'].attrs['Units'] = 'None'

        fl.create_dataset(f'{rg}/{snap}/Particle/BH_mstellar_{radius}pkpc_cylinder', data = mstellar_5pkpc, dtype = np.float64)
        fl[f'{rg}/{snap}/Particle/BH_mstellar_{radius}pkpc_cylinder'].attrs['Description'] = 'Mass of stellar particles within 5pkpc of BH - NOT SUBFIND!'
        fl[f'{rg}/{snap}/Particle/BH_mstellar_{radius}pkpc_cylinder'].attrs['Units'] = 'None'

        fl.create_dataset(f'{rg}/{snap}/Particle/BH_ngas_{radius}pkpc_cylinder', data = ngas_5pkpc, dtype = np.float64)
        fl[f'{rg}/{snap}/Particle/BH_ngas_{radius}pkpc_cylinder'].attrs['Description'] = 'No. of gas particles within 5pkpc of BH - NOT SUBFIND!'
        fl[f'{rg}/{snap}/Particle/BH_ngas_{radius}pkpc_cylinder'].attrs['Units'] = 'None'

        fl.create_dataset(f'{rg}/{snap}/Particle/BH_mgas_{radius}pkpc_cylinder', data = mgas_5pkpc, dtype = np.float64)
        fl[f'{rg}/{snap}/Particle/BH_mgas_{radius}pkpc_cylinder'].attrs['Description'] = 'Mass of gas particles within 5pkpc of BH - NOT SUBFIND!'
        fl[f'{rg}/{snap}/Particle/BH_mgas_{radius}pkpc_cylinder'].attrs['Units'] = 'None'

        fl.create_dataset(f'{rg}/{snap}/Particle/BH_meanfesc_{radius}pkpc_cylinder', data = mean_fesc_5pkpc, dtype = np.float64)
        fl[f'{rg}/{snap}/Particle/BH_meanfesc_{radius}pkpc_cylinder'].attrs['Description'] = 'Mean escape fraction of stellar particls within 5pkpc of BH - NOT SUBFIND!'
        fl[f'{rg}/{snap}/Particle/BH_meanfesc_{radius}pkpc_cylinder'].attrs['Units'] = 'None'
...
if __name__ == "__main__":
    ...
    if s_nbours=={}:
        ...
        nsf_bhids = []
        nsf_bhmass = []
        save_data(newfile, rg, snap, nstellar_5pkpc, nsf_bhids, nsf_bhmass,
              mstellar_5pkpc, ngas_5pkpc, mgas_5pkpc, mean_fesc_5pkpc)
        exit()
    ...
    M_sun = 1.99 * 10**30 # [kg]
    M_H = 1.67 * 10**-27 # [kg]
    s_hlos = s_hlos * (M_sun/M_H) # [H atoms/pc^2]
    sigma = 6.3 * 10**-18 # photoionisation cross section [cm^2]
    sigma = sigma / (3.1*10**18)**2 # [pc^2]
    fesc = np.exp(-sigma*s_hlos)
    print(f'fesc min, max: {np.amin(fesc), np.amax(fesc)}')
    ...
    for ii in range(len(g_nbours)): # for each galaxy
        ...
        # NOTE: cut out cylinder with its cross-section facing the z-dir
        #       centered on the bh with a radius of 5pkpc

        # associate direction with column in coord array
        xdir, ydir, zdir = 0, 1, 2

        print('shape of coords:', gal_scoords.shape, gal_gcoords.shape,
              gal_bhcoords.shape)

        # hence, we only care about the x, y coords
        gal_scoords = gal_scoords[:,xdir:zdir]
        gal_gcoords = gal_gcoords[:,xdir:zdir]
        gal_bhcoords = gal_bhcoords[:,xdir:zdir]

        print('new shape of coords:', gal_scoords.shape, gal_gcoords.shape,
              gal_bhcoords.shape)
        ...
        s_query = tree.query_ball_point(gal_scoords, r=0.01, p=2)
        g_query = tree.query_ball_point(gal_gcoords, r=0.01, p=2)
